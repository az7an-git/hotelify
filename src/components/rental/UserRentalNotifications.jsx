...
function UserRentalNotifications({ rentalVehicles, handleDelete, loading, deletingId }) {
  return (
    <div className="p-4 sm:p-6">
      {loading.rental ? (
        <Loader msg={"Fetching rental Updates for you"} />
          ...
          {rentalVehicles.map((vehicleNotification, i) => (
            ...
            >
              <div className="flex-1 flex flex-col justify-between mb-4">
                <div>
                  <p className="font-bold text-white text-base mb-1">{vehicleNotification.userName}</p>
                  <p className="text-emerald-400 font-medium text-sm mb-3">{vehicleNotification.message}</p>
                  <p className="text-sm text-slate-300">
                    <span className="font-medium text-slate-400">Ordered Vehicle: </span>
                    <span className="text-slate-100 font-semibold">{vehicleNotification.orderedVehicle}</span>
                  </p>
                </div>
              </div>
              ...
              >
                {deletingId === vehicleNotification.id ? "Deleting..." : "Clear Notification"}
              </button>
            </div>
          ))}
        </div>
