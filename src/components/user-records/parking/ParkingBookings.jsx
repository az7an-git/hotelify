...
function ParkingBookings({ bookingFields, bookings, activeTab }) {
        ...
        <table className="w-full text-left whitespace-nowrap">
          ...
          <tbody className="divide-y divide-white/40">
            {bookings.map((booking, index) => (
              <tr key={booking.id || index} className="hover:bg-white/40 transition-colors duration-200">
                {bookingFields[activeTab].map((field) => (
                  <td key={field} className="px-6 py-5 font-bold text-slate-800 capitalize">
                    {field === "status" ? (
                      ...
                      <span className="text-amber-700 font-semibold">${booking[field]}</span>
