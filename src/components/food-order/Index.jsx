  ...
  return loading ? (
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      <div className="border-b border-slate-800/80 pb-4 mb-6">
        ...
        <p className="text-slate-400 text-sm mt-1">Select from our chef-crafted gourmet menu options.</p>
      </div>
      <Tabs
      ...
      {foodItems.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredItems && filteredItems.length > 0 ? (
            ...
            <div className="col-span-full text-center py-12 bg-slate-900/50 rounded-2xl border border-slate-800/80 text-slate-400 text-base">
              No Items Found in this Category try Choosing Something else please
            </div>
          )}
        </div>
      ) : (
        <div className="text-center py-12 bg-slate-900/50 rounded-2xl border border-slate-800/80 text-slate-400 text-base">
          No items Found
        </div>
      )}
      <BookingForm
